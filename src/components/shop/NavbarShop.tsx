  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-[#222222] bg-[#0A0A0A]/90 backdrop-blur-md">
      <div className="container mx-auto flex items-center justify-between px-4 py-3 lg:px-8">
        ...
        {/* Buscador (solo en desktop, en móvil se puede colapsar) */}
        <form onSubmit={handleSearch} className="hidden md:block flex-1 max-w-md mx-6">
          <div className="relative">
            ...
            />
            <svg
              className="absolute left-3 top-2.5 h-4 w-4 text-[#6B7280]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      <form onSubmit={handleSearch} className="md:hidden px-4 pb-3">
        <div className="relative">
          <input
            ...
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Buscar productos..."
            className="w-full rounded-full border border-[#333333] bg-[#111111] py-2 pl-10 pr-4 text-sm text-white placeholder:text-[#6B7280]"
          />
          <svg className="absolute left-3 top-2.5 h-4 w-4 text-[#6B7280]"/>
        </div>
      </form>
    </header>
  );
