...
export async function generateCustomerToken(customerId: string, email: string): Promise<string> {
  ...
  console.log(`[generateCustomerToken] Secreto obtenido (longitud: ${secret.byteLength} bytes)`);
...
export async function verifyCustomerToken(token: string): Promise<CustomerPayload | null> {
  ...
  try {
    const secret = getCustomerSecret();
    console.log(`[verifyCustomerToken] Secreto para verificar (longitud: ${secret.byteLength} bytes)`);
...
export async function getCustomerFromCookies() {
  ...
  console.log(`[getCustomerFromCookies] Payload válido: id ${payload.id}, email ${payload.email}`);
  ...
  if (!customer) {
    ...
    console.log(`[getCustomerFromCookies] Cliente encontrado: ${customer.email} (${customer.name})`);
  }
  return customer;
}