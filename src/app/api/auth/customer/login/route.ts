...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user) {
      ...
      response.cookies.set("bt_auth", token, { httpOnly: true, path: "/", sameSite: "strict", secure: process.env.NODE_ENV === "production", maxAge: rememberMe ? 60 * 60 * 24 * 30 : 60 * 60 * 8 });
      return response;
    }
    ...
    if (customer) {
      ...
      response.cookies.set("bt_customer_token", token, { httpOnly: true, path: "/", sameSite: "strict", secure: process.env.NODE_ENV === "production", maxAge: rememberMe ? 60 * 60 * 24 * 7 : undefined });
      return response;
    }
